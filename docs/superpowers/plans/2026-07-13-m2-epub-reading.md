...
- **Recorded deviations:** whole-epub serving instead of per-resource unzip (justified at the top of Phase B); Locator field `spineIndex` alongside the spec's `spineHref` (both carried; index is what foliate-js round-trips natively); `progressionInChapter` currently null from the bridge (renderer-internal; display-only field, documented). **Added by final review:** (a) `Locator.textContext` from the spec's composite record is not yet a field — deferred to M3 where annotation re-anchoring needs it (additive Codable optional, no migration); (b) the bridge runtime is served as a `<script type="module">` from the scheme rather than injected via `WKUserScript` at document start — CSP-friendlier, same single channel; (c) sweep phase numbering skips 4 (folded into phase 2's known-book branch) — cosmetic, renumber when the sweep is next touched.
